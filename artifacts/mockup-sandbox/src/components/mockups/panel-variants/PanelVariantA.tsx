/* Panel Page — Variant A: Compact Metric Strip with SVG icons */
...
const IconRam = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <path d="M6 19V5a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v14"/>
    <path d="M3 19h18"/>
...
function RadialMini({ pct, color, size = 34 }: { pct: number; color: string; size?: number }) {
  const r    = (size - 5) / 2;
  const cx   = size / 2;
  const cy   = size / 2;
  ...
  return (
    <svg width={size} height={size} style={{ transform: 'rotate(-90deg)', flexShrink: 0 }}>
      <circle cx={cx} cy={cy} r={r} fill="none" stroke={DIM} strokeWidth={2.5} />
...
const stats = [
  { label: 'CPU',     value: '34%',    sub: 'of vCPU',   color: BLUE,  pct: 34,  Icon: IconCpu },
  { label: 'RAM',     value: '178 MB', sub: 'of 512 MB', color: GREEN, pct: 35,  Icon: IconRam },
  { label: 'Uptime',  value: '3h 22m', sub: 'running',   color: GREEN, pct: 100, Icon: IconClock },
  { label: 'Restarts',value: '2',      sub: 'today',     color: AMBER, pct: 20,  Icon: IconRotate },
];
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'system-ui, -apple-system, sans-serif', color: '#fff' }}>
      ...
      <div style={{ padding: '10px 14px 0', overflowX: 'auto' }}>
        <div style={{ display: 'flex', gap: 7, minWidth: 'max-content', paddingBottom: 10 }}>
          {stats.map(s => (
            <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 12px', borderRadius: 12, background: CARD, border: `1px solid ${BORDER}`, minWidth: 110 }}>
              <RadialMini pct={s.pct} color={s.color} />
              <div>
                <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: -0.5, color: '#fff', lineHeight: 1.1 }}>{s.value}</div>
                <div style={{ fontSize: 9.5, color: MUTED, marginTop: 2 }}>{s.label}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', borderBottom: `1px solid ${BORDER}` }}>
        {tabs.map((t, i) => (
          <button key={t.label} style={{
            padding: '10px 14px', fontSize: 12.5, fontWeight: i === 0 ? 700 : 500,
            ...
            {t.label}
            {t.isConsole && (
              <span style={{ marginLeft: 2, opacity: 0.5, display: 'flex', alignItems: 'center' }} title="Expand">
                <IconExpand />
              </span>
            )}
          </button>
