/* Panel Page — Variant B: Two-row metric cards (2×2 with bar gauges)
   Dark #0c0c0f bg • larger, more premium metric cards */
...
function Bar({ pct, color }: { pct: number; color: string }) {
  return (
    <div style={{ width: '100%', height: 4, borderRadius: 2, background: DIM, overflow: 'hidden', marginTop: 8 }}>
      <div style={{ height: '100%', width: `${pct}%`, borderRadius: 2, background: color, transition: 'width 0.5s' }} />
    </div>
  );
}

function MetCard({ label, value, sub, color, pct, accent }: {
  label: string; value: string; sub: string; color: string; pct: number; accent: string;
}) {
  return (
    <div style={{ flex: 1, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 14px 12px', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, borderRadius: '14px 0 0 14px', background: accent }} />
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: 11, color: MUTED, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 4 }}>{label}</div>
          <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: -1, color: '#fff', lineHeight: 1 }}>{value}</div>
        ...
        <div style={{ width: 36, height: 36, borderRadius: 10, background: `${color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
          {label === 'CPU' ? '⚡' : label === 'RAM' ? '💾' : label === 'Uptime' ? '🟢' : '↺'}
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'system-ui, -apple-system, sans-serif', color: '#fff' }}>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <MetCard label="CPU" value="34%" sub="of vCPU" color={BLUE} pct={34} accent={BLUE} />
          <MetCard label="RAM" value="178 MB" sub="of 512 MB" color={GREEN} pct={35} accent={GREEN} />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <MetCard label="Uptime" value="3h 22m" sub="running" color={GREEN} pct={100} accent={GREEN} />
          <MetCard label="Restarts" value="2" sub="total" color={AMBER} pct={20} accent={AMBER} />
        </div>
      </div>
      ...
      <div style={{ background: '#0d1117', margin: 12, borderRadius: 12, border: `1px solid ${BORDER}`, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '10px 14px', fontFamily: 'monospace', fontSize: 12, lineHeight: 1.7 }}>
          ...
          <div style={{ color: '#34d399', marginTop: 8 }}>$ _</div>
        </div>
      </div>
    </div>
  );
